...
if (!isset($_POST['submit'])) {
    ...
    $sqlph = "SELECT phone_number from user_phone where user_id = $id";
    ...
    $sqlph = "SELECT phone_number from user_phone where user_id = $id";
    $resultph = mysqli_query($conn, $sqlph);
    $i = mysqli_num_rows($resultph);
    for ($j = 1; $j <= $i; $j++) {
        $res['phone_number_' . $j] = $_POST['phone_number_' . $j];
    }
    ...
    for ($j = 1; $j <= $i; $j++) {
        if(strlen($res['phone_number_' . $j])!=10){
            $valid=false;
            $error['phone_number'] = 'Phone numbers should have exactly 10 characters';
        }
    }
    ...
    if ($valid) {
        ...
        $phoneNumbers = array();
        mysqli_data_seek($resultph, 0); // Reset the result pointer to the beginning
        while ($resph = mysqli_fetch_assoc($resultph)) {
            $phoneNumbers[] = $resph['phone_number'];
        }
    
        $j = 1;
        foreach ($phoneNumbers as $phoneNumber) {
            if ($phoneNumber != $res['phone_number_' . $j]) {
                $upd_ph = "UPDATE user_phone SET phone_number = '{$res['phone_number_' . $j]}' WHERE user_id = $id AND phone_number = '{$phoneNumber}';";
                $upd_ph_res = mysqli_query($conn, $upd_ph);
                if (!$upd_ph_res) {
                    mysqli_error($conn);
                    header('Location: user_info.php?upd=0');
                }
