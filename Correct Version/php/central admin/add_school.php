...
if (isset($_POST["submit"])) {
    ...
    $sql_unique="SELECT user_id from users where username = '{$res['username']}';";
    $res_unique = mysqli_query($conn, $sql_unique);

    if(mysqli_num_rows($res_unique)!=0){
        $correct = false;
        $errors['username'] = 'This username has already been taken. Please select a different one.';
    }
    ...
    if($phone && strlen($res['phone_number'])!=10){
            $correct = false;
...
<html>
  ...
  <body>
    <div class="login-box-school" style="margin-top:50%;>
        <div class="login-box-school-container">
            ...
            <form class="login-form" action="add_school.php?>" method="POST">
                ...
                <input type="text" name="phone_number value="<?php echo $res['phone_number'];?>"/>
