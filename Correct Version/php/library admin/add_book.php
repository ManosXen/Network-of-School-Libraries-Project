        ...
        elseif($copies<0){
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                try{        
                    ...
                    $school="SELECT school_id from library where school_id<>$schoolID";
                    $result_school=mysqli_query($conn,$school);
                    $schoollist = mysqli_fetch_all($result_school);
                    $avail=0;
                    foreach ($schoollist as $schoolItem) {
                        $schoolID = $schoolItem[0]; 
                        $query1 = "INSERT INTO book_owner(school_id, ISBN, avail_copies) VALUES ($schoolID, '$ISBN', $avail)";
                        if (!mysqli_query($conn, $query1)) {
                            throw new Exception("Error in insertion of book");
                        }
                    }
...
<header>
    ...
    <div class="login-box" style="margin-top:22%">
