...
if(isset($_POST['submit_creds'])){
    ...
    if(empty($username)  || empty($first_name) || empty($last_name) || empty($birth_date)  || empty($schoolID)){
        $error = "everything except phone number is required.";
    }
    elseif(strlen($username)<6){
        $error="Username must be at least 6 charachters";
    }
...
<header>
    ...
    <a href="lib_admin_appl.php?id=<?php echo $userID;?>"><button class="inline-button" name="submit_creds">Library Admin Applications</button><butto</a>    
    </div>
